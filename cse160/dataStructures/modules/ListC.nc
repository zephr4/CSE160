...
implementation{
	...
	//Implemented: Here we are collapsing the list so that we remove specfics nodes
	command t List.pop(int position) {
		uint8_t i = 0;
		t returnVal;
		returnVal = container[0];
	
		if (size > 0) {//collapse data back in after popping @ position
